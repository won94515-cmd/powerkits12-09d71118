...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return new Response(JSON.stringify({ ok: false, error: "Discord credentials not configured" }), {
    ...
    if (!guild) {
      return new Response(JSON.stringify({ ok: false, error: "No guild — bot was not added to a server" }), {
        status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({
      success: true, guild: { id: guild.id, name: guild.name, icon: guild.icon }
